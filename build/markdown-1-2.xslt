...
<xsl:stylesheet version="2.0" 
  ...
  <xsl:template match="/">
    <xsl:result-document method="text" href="markdown/Aquino_Summa-1-2.md">
      <xsl:text>---&#10;</xsl:text>
      <xsl:text>title: | &#10; | SUMMA THEOLOGIÆ&#10; | Prima Pars - Prima Secundæ&#10;</xsl:text>
      ...
      <xsl:apply-templates select="summa/liber[@title and (number(@index)=1 or number(@index)=2)]"/>
      <xsl:text>\clearpage&#10;</xsl:text>
  ...
  <xsl:template match="liber[@title]">
    ...
    <xsl:text>&#10;</xsl:text>
    <xsl:apply-templates select="quaestio[articulus/lemma/nl[not(abest)]]"/>    
  </xsl:template>
